...
def testlist():
    frm = Question('Хто написав картину «Мона Ліза»?', 'Леонардо да Вінчі', 'Мікеланджело', 'Рембрандт', 'Ван Гог')
    questions_listmodel.form_list.append(frm)
    frm = Question('Хто був першим президентом США?', 'Джордж Вашингтон', 'Авраам Лінкольн', 'Томас Джефферсон', 'Бенджамін Франклін')
    questions_listmodel.form_list.append(frm)
    frm = Question('Яка річка є найдовшою в світі?', 'Ніл', 'Амазонка', 'Міссісіпі', 'Янцзи')
    questions_listmodel.form_list.append(frm)
    frm = Question('Що таке фотон?', 'Частинка світла', 'Електричний заряд', 'Магнітне поле', 'Вітер')
    questions_listmodel.form_list.append(frm)
...
apply_gradient_background(win_main)
apply_gradient_background(win_card)
